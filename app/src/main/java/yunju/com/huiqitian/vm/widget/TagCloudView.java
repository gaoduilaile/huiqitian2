...
public class TagCloudView extends ViewGroup {
    ...
    private static final int SELCECT_TEXT_COLOR = R.color.color_e51c23;//选中后的标签颜色
     ...
     *设置标签包括数量
     */
    public void setCommentTags(List<String>tagList,List<Integer>totalList){
        this.tags = tagList;
        this.removeAllViews();
        ...
        if (tags != null && tags.size() > 0) {
            for (int i = 0; i < tags.size(); i++) {
                ...
                if (mTagResId == DEFAULT_TAG_RESID) {
                    ...
                    tagView.setTextSize(TypedValue.COMPLEX_UNIT_SP, mTagSize);
                    tagView.setTextColor(mTagColor);
                    spanStr = new StringFormatUtil(getContext(), tagList.get(i)+"("+totalList.get(i)+")",
                            totalList.get(i)+"", R.color.tv_exist_account).fillColor();
                }
                LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
                tagView.setLayoutParams(layoutParams);
                tagView.setText(spanStr.getResult());
                tagView.setTag(TYPE_TEXT_NORMAL);
                tagView.setGravity(Gravity.CENTER);
                final int finalI = i;
                tagView.setOnClickListener(new OnClickListener() {
